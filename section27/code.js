...
function randOneToOneHundred(){
     let newRand = randomGenerator(0,100)
...
// Returns Long lorem Ipsum String
...
function loremSentance(){
    let numberOfSentences=  document.getElementById("loremSentance").value
    // split array containing the sentances
    let newLoremSentance =lorem.split(".")

    let newLorem=""

    for (let index = 0; index < numberOfSentences; index++) {
        newLorem=newLorem+newLoremSentance[index]+"."
...
function loremChars(){
    ...
    for (let index = 0; index < numberOfSentences; index++) {
        newLoremChar=newLoremChar+newLoremCharacters[index]
        
    }
    //console.log(newLoremChar)
    document.getElementById("p5").innerHTML =newLoremChar
}
...
function newTime(){
    ...
    let myNewTime = hours+":"+minute+" "+amPm
...
function compareWords(){
    ...
    if(word1.length==word2.length){
        msg="Both words are same length"
    }else{
        msg="words lengths are different. Word1 :"+word1.length+"   Word2: "+word2.length
    }
...
function passwordGen(){
    ...
    for (let i = 0; i <=12 ; i++) {
        let num = randomGenerator(0,chars.length)
